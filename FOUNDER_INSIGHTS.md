...
across Sprints 1 through 1.2 and carried forward into every later sprint.
...
picked up loot, claimed a ship, switched builds — in under two minutes, from
opening the app to a Captain's Log entry existing. Every added field,
dropdown, or confirmation step on Quick Update is a tax against this rule and
has to earn its place. It's also why Quick Update's "What changed?" list
tracks what players actually do (install, remove, move, add to hangar,
switch build) instead of a generic catch-all.
...
it recommends Keep or Ignore, and now finds the item for you as you type
instead of demanding exact spelling. Mission Control doesn't just show
ships — it ranks Top Priority. Procurement List doesn't just show gaps — it
tells the player what to go get, fleet-wide, grouped by component.
...
wants it, sorted alphabetically so it reads like a shopping list — is
decision support. It answers "what should I buy or go get next," not just
"what does this one ship lack."
...
that's the end of the workflow for that item. There's also no "Claimed Ship"
or "Vendor Loot" Quick Update type anymore — claiming a ship doesn't change
what the player needs to do next, so it isn't worth a dedicated workflow.

## Ships are claimable; components are location-bound

A ship can be claimed at almost any terminal in the verse — its physical
location isn't a meaningful, stable fact worth tracking or displaying.
Components are different: what's sitting in your hangar at a given outpost
actually constrains what you can do right now. That's why Ship Detail has no
location field at all, while Hangar Inventory is the place a future Location
field belongs.

## Complexity belongs in the engine, simplicity belongs in the UI

Hardpoint status logic — Factory seeds Installed, Installed can drift from
both Factory and Target, and a part that's been swapped but doesn't match
the Target shows "Upgrade Available" rather than "Missing" — is genuinely
subtle. The player never has to think about that logic; they just see a
green, yellow, or red badge. The engine carries the nuance so the interface
doesn't have to.
