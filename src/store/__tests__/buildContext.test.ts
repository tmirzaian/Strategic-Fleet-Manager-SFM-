...
describe('Mission Context installs (Golden Scenario F, Alpha 2.2 shared Installed Loadout)', () => {
  it('30. Mission Context selection alone never changes activeBuildId', () => {
    ...
    useFleetStore.getState().installComponent('ghost', 'FR-66', 'Shield 1', 'ghost-escort')
  ...
  it('31. installing FR-66 under the Escort Mission context physically changes the shared Installed Loadout for that slot', () => {
    const escortShieldBefore = useFleetStore.getState().hardpoints.find((h) => h.buildId === 'ghost-escort' && h.slotLabel === 'Shield 1')!
    expect(escortShieldBefore.installedItem).toBe('Mirage')

    const result = useFleetStore.getState().installComponent('ghost', 'FR-66', 'Shield 1', 'ghost-escort')
    expect(result.matched).toBe(true)

    const escortShieldAfter = useFleetStore.getState().hardpoints.find((h) => h.buildId === 'ghost-escort' && h.slotLabel === 'Shield 1')!
  ...
  it('32 (Alpha 2.2 fix): installing under a non-active Mission context updates Installed for EVERY Mission sharing that slot, since Installed Loadout is real shared physical state — this is the bug this sprint fixes, not a regression', () => {
    const stealthShieldBefore = useFleetStore.getState().hardpoints.find((h) => h.buildId === 'ghost-stealth' && h.slotLabel === 'Shield 1')!
    ...
    const stealthShieldAfter = useFleetStore.getState().hardpoints.find((h) => h.buildId === 'ghost-stealth' && h.slotLabel === 'Shield 1')!
  ...
  it('a shared InstalledLoadout entry exists and reflects the latest physical install for the slot', () => {
    useFleetStore.getState().installComponent('ghost', 'FR-66', 'Shield 1', 'ghost-escort')
    const entry = useFleetStore.getState().installedLoadouts.find((e) => e.shipId === 'ghost' && e.slotLabel === 'Shield 1')
