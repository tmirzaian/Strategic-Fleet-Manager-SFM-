...
import FlagshipEnvironmentLayer from './components/flagship/FlagshipEnvironmentLayer'
import FlagshipFrame from './components/flagship/FlagshipFrame'
import FlagshipThreshold from './components/flagship/FlagshipThreshold'
...
export default function App() {
  ...
  // EWO-115 (Part B/F) established Mission Control as the first
  // full-viewport Flagship Environment consumer; EWO-116 (Part B/C) adds
  // Decision Center as the second, migrated Station. Every other route
  // still falls back to the ambient gradient alone (FlagshipEnvironmentLayer's
  // own default). This lookup is App.tsx's own route knowledge driving a
  // typed asset-identifier prop — not a special-case Sidebar/Station
  // implementation (EWO-115 Part I's own explicit requirement, still
  // honored: no route below required any Sidebar-level change).
  const ROUTE_FLAGSHIP_ENVIRONMENT: Partial<Record<string, EnvironmentId>> = {
    '/': 'mission-control',
    '/decision-center': 'decision-center',
  }
  const flagshipEnvironmentId = ROUTE_FLAGSHIP_ENVIRONMENT[location.pathname]

  return (
    <div className="relative flex min-h-screen bg-bg text-white">
      {/* EWO-115 (Part B) — the flagship's persistent backdrop: a true
          position:fixed layer occupying the entire viewport, rendered
          before every flex sibling below so it is never confined to
          <main>'s own bounds and extends beneath the Sidebar's column.
          Sidebar/Threshold/main all render above it (relative z-10). */}
      <FlagshipEnvironmentLayer environmentId={flagshipEnvironmentId} />
      <Sidebar />
      {/* EWO-113 (Objective 2) — the architectural threshold immediately to
          the right of the Sidebar. Persistent and unconditional: a sibling
          of <main>, entirely outside the routed <Suspense> boundary, so it
          never remounts or flickers between Stations. */}
      <FlagshipThreshold />
      ...
      <main className="relative z-10 flex-1 min-w-0 max-w-[1400px] flex flex-col">
        {/* EWO-113 (Objective 3) — wraps only the routed content, never
            AppFooter, in the Flagship's shared layout column, now sitting
            above FlagshipEnvironmentLayer's own full-viewport backdrop. */}
        <FlagshipFrame>
