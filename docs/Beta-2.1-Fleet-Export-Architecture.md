...
|---|---|
| **Import** | ✅ **Implemented — EWO-094, commit `78dd3cb`.** Parse uploaded file (`parseFleetImportFile`, envelope-level only) → schema-version compatibility check → `migrateFleetPersistedState` → `mergeFleetPersistedState`, all in memory → **Preview** (the Commander inspects that already-hydrated result — decided by Chief Architect Certification, EWO-093) → on confirmation, `replaceFleetFromImport` commits via the store's own `set()`, which the existing `persist` middleware writes to `localStorage` exactly like any other mutation. No direct `localStorage.setItem`/reload was needed in the end — calling the extracted functions directly and committing in-process proved simpler and avoids a jarring full-page reload. Replace-vs-merge semantics beyond a full Replace remain deferred to a future EWO. |
| **Backup** | Call the same `buildFleetExportSnapshot` + `serializeFleetExportEnvelope` pair Export already uses, on a schedule or Commander action, writing to a rotating/dated store (e.g. a small IndexedDB table or a sequence of downloaded files) instead of a single manual download. No new serialization — only a new trigger and destination. **Not yet built:** EWO-094's `replaceFleetFromImport` already captures an in-memory recovery snapshot via this exact mechanism before every Replace, but there is still no Commander-facing UI to view or restore it — carried forward to the next Backup/Recovery work order, per Chief Architect direction on EWO-094's certification. |
| **Restore** | Exactly the Import mechanism above, sourced from a specific stored backup entry (or the in-memory EWO-094 recovery snapshot) instead of a Commander-picked file. |
