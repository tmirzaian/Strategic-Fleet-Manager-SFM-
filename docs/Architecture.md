...
**`src/components/ShipCard.tsx` is the single canonical Ship Card for Beta 1.0** (EWO-032, corrected/finalized EWO-033) — Fleet Dashboard, Mission Control, and every future ship-grid surface (Fleet Roadmap, Squadron views) render this one component, with one shared structural dimension contract (four reserved-height regions, uniform regardless of content) and one shared stock role/focus resolver (`src/utils/shipIdentityLine.ts`) — no page maintains its own card layout, own dimension rules, or own role/focus logic. Priority is page-level wrapper context via the shared `PriorityLabel` component, never embedded in the card itself: Fleet Dashboard shows every Fleet Asset's own priority value in Card view; Mission Control shows the Top 4. The prior Mission Control-only card (`ShipRecordCard`/`PriorityCard`) is retired (kept on disk, not deleted, pending Commander verification). See `docs/UI_ARCHITECTURE.md` §19/§19.1.

**Ship imagery is resolved through one canonical path everywhere it appears** (`src/utils/resolveShipImage.ts` + the Commander-maintained `src/data/shipImageRegistry.ts`, finalized EWO-033A) — Ship Detail, `ShipCard`, and therefore Fleet Dashboard and Mission Control all consume the same resolved `imageUrl`/fallback, with real photography and the universal fallback artwork (`SHIP_PLACEHOLDER_URL`) sharing the same frame-filling presentation and fixed dimensions. See `docs/UI_ARCHITECTURE.md` §19.2 and `docs/ASSET_PIPELINE.md`.
